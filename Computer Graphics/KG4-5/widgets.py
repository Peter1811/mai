...
def change_small_axis(value):
    ...
    global height
    global height_slide
    b_axis = return_big_axis()
    coef = float(value) / float(b_axis)
    if height_slide > 1 or small_axis > value:
        height_slide = height - (height * coef)
        small_axis = value
...
def change_height(value):
    ...
    global big_axis
    global small_axis
    coef = float(small_axis) / float(big_axis)
    height = float(value)
    if height_slide > 1 or small_axis > value:
       height_slide = height - (height * coef)
...
def make_window():
    ...
    label_big_axis = tk.Label(window, text="Радиус основания")
    label_big_axis.grid(row=0, column=0)
    scale_big_axis.grid(row=0, column=1)

    scale_small_axis = tk.Scale(window, from_=0, to=9.9, resolution=0.1, orient=HORIZONTAL, command=change_small_axis)
    label_small_axis = tk.Label(window, text="Радиус сечения")
